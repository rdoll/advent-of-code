...
class TopologyMap:
    ...
    def __init__(self):
        ...
        self.trailheads: dict[tuple[int, int], set[tuple[int, int]]] = {}
    ...
    def follow_trail(self, next_height: int, row: int, col: int) -> set[tuple[int, int]]:
        if row not in range(0, self.rows) or col not in range(0, self.columns):
            return set()

        if self.map[row][col] == next_height:
            if next_height == 9:
                return {(row, col)}  # trail ended here

            all_ends = set()
            for offset in TopologyMap.TRAIL_OFFSETS:
                offset_ends = self.follow_trail(next_height + 1, row + offset[0], col + offset[1])
                all_ends.update(offset_ends)
            return all_ends

        return set()  # this trail cannot continue

    def add_trails(self, head_row: int, head_col: int, ends: set[tuple[int, int]]):
        head_pos = (head_row, head_col)
        if len(ends) > 0:
            logging.debug(f'Trailhead at {head_pos} ended at {ends}')
            if not head_pos in self.trailheads:
                self.trailheads[head_pos] = set()
            self.trailheads[head_pos].update(ends)
    ...
    def find_trails(self):
        for r in range(0, self.rows):
            for c in range(0, self.columns):
                if self.map[r][c] == 0:
                    for offset in TopologyMap.TRAIL_OFFSETS:
                        ends = self.follow_trail(1, r + offset[0], c + offset[1])
                        self.add_trails(r, c, ends)

    def trailheads_score(self) -> int:
        score: int = 0
        for trail_ends in self.trailheads.values():
