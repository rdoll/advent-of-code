...
class ClawMachine:
    ...
    def find_min_tokens(self) -> int | None:
        max_as: int = min(self.px // self.ax, self.py // self.ay, ClawMachine.MAX_BUTTONS)
        max_bs: int = min(self.px // self.bx, self.py // self.by, ClawMachine.MAX_BUTTONS)

        min_tokens: int = 999999999999999
        for num_as in range(0, max_as + 1):
            for num_bs in range(0, max_bs + 1):
                claw_x = num_as * self.ax + num_bs * self.bx
                claw_y = num_as * self.ay + num_bs * self.by
                if claw_x == self.px and claw_y == self.py:
                    tokens = num_as * ClawMachine.TOKENS_PER_A + num_bs * ClawMachine.TOKENS_PER_B
                    if tokens < min_tokens:
                        min_tokens = tokens
                    logging.debug(f'Prize with {num_as} As and {num_bs} Bs for {tokens} tokens on machine {self}')

        return None if min_tokens == 999999999999999 else min_tokens
