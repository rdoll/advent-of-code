...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                                     description='Advent of Code 2025 Day 09 Part 2 Solution')
...
def read_positions(input_file: TextIOWrapper) -> List[Tuple[int, int]]:
    ...
    min_x, min_y = min(positions, key=lambda p: p[0])
    max_x, max_y = max(positions, key=lambda p: p[1])
...
def main() -> int:
    ...
    red_tile_polygon = Polygon(red_tile_positions)
    ...
    for pos1 in red_tile_positions:
        for pos2 in red_tile_positions:
            ...
            if area > largest_area:
                box: Polygon = shapely.box(pos1[0], pos1[1], pos2[0], pos2[1])
                if shapely.covers(red_tile_polygon, box):
                    largest_area = area
